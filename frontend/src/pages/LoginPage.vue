      ...
      <div class="container login-shell">
        <div class="login-grid">
          ...
          <div class="card login-card">
            <div class="mode-tabs">
              <button type="button" class="chip" :class="{ active: mode === 'login' }" @click="switchMode('login')">
                登录
              </button>
              <button type="button" class="chip" :class="{ active: mode === 'register' }" @click="switchMode('register')">
                注册
              </button>
            </div>

          <div v-if="mode === 'register'" class="role-tabs">
            <button type="button" class="chip" :class="{ active: role === 'student' }" @click="switchRole('student')">
              学生注册
            </button>
            <button type="button" class="chip" :class="{ active: role === 'company' }" @click="switchRole('company')">
              企业注册
            </button>
          </div>

          <template v-if="mode === 'login'">
            <form v-if="!showResetForm" @submit.prevent="handleLogin" class="login-form">
              <label>
                邮箱
              ...
              <button type="button" class="btn-link" @click="openResetForm">忘记密码？</button>
            </form>
            ...
            <form v-else @submit.prevent="handleResetPassword" class="login-form">
              ...
              <div class="reset-row">
                ...
                <button type="button" class="btn btn-outline code-btn" :disabled="resetCooldown > 0" @click="sendResetVerifyCode">{{ resetCooldown > 0 ? `${resetCooldown}s` : '发送验证码' }}</button>
              </div>
              <label>
                新密码
              ...
              <div class="reset-row">
                <button class="btn" type="submit" :disabled="loading">{{ loading ? '重置中...' : '重置密码' }}</button>
                <button type="button" class="btn btn-outline" @click="closeResetForm">返回登录</button>
              </div>
            </form>
          </template>
          ...
          <form v-else @submit.prevent="handleRegister" class="login-form">
            ...
            <div v-if="mode === 'login' && !showResetForm" class="quick-login">
              <p class="mono">演示账号（密码统一 123456）：</p>
              <div class="quick-actions">
                <button type="button" class="chip" @click="fillDemo('student')">学生 student@test.com</button>
                <button type="button" class="chip" @click="fillDemo('company')">企业 company@test.com</button>
                <button type="button" class="chip" @click="fillDemo('admin')">管理员 admin@test.com</button>
              </div>
            </div>
          </div>
        </div>
...
function closeResetForm() {
  ...
  resetForm.value = { email: '', code: '', new_password: '' };
}
